package io.github.defective4.pi.pcd8544.font;

import java.util.ArrayList;
import java.util.Collection;
import java.util.Collections;
import java.util.List;
import java.util.Objects;

public class BitmapFont {

    public static final BitmapFont DEFAULT_FONT = new BitmapFont(new int[] {
            
    });

    private final Collection<Integer> font;

    public BitmapFont(int[] font) {
        Objects.requireNonNull(font);
        if (font.length % 5 != 0) throw new IllegalArgumentException("Font array length must be dividable by 5");
        List<Integer> lst = new ArrayList<>();
        for (int i : font) lst.add(i);
        this.font = Collections.unmodifiableCollection(lst);
    }

    public int[] getFont() {
        return font.stream().mapToInt(e -> e).toArray();
    }

}
